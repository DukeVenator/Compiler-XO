...
function InstallXOPlugins {
	...
	if [[ "$PLUGINS" ]] && [[ ! -z "$PLUGINS" ]]; then
		...
		if [[ "$PLUGINS" == "all" ]]; then
			echo
			echo "Installing all da things as defined in the PLUGINS var in dat damn complicated config"
...
function InstallXO {
	...
	echo
	echo This is gonna take a while. Grab a nice cup of bloody patience and come back with a time machine"
	echo
